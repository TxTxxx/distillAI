...
components:
  ...
  source-chip:
    ...
    textColor: "{colors.ink}"
    padding: "calc(3 * var(--layout-unit, 1px)) calc(8 * var(--layout-unit, 1px))"
  agent-control:
    textColor: "{colors.ink}"
...
本文件于 2026-09-13 按当前代码刷新，并抽查本地首页的浏览器计算样式与画面。由实际 `src/styles.css`、`src/App.tsx` 与 Launch、QuestionLibrary、GlyphField、Room、AgentEditor、Settings 组件提取。方向来自用户选择的「文字流动」，最终 index 构图由用户授权选择，批准图为 `.impeccable/mocks/index.png`。这是实现记录，不是质量验收证书。
...
- **Body**：根字号为 16px，前言 body 专指 Markdown 长文的 17px 阅读样式；手机仍保留 17px，行高为 1.9。单条讨论正文最大宽度 780px。
...
| ≤680px  | 顶栏 68px，导航使用具名图标，模型与声音保留文字；首页先起草后题库。起草区水平内距 20px，动作高 48px；助教改为覆盖全屏的面板。编辑角色变横向标签，设置表单变单列。 |

讨论室通过弹性布局占据顶栏和错误提示以外的视口高度，主讨论区独立滚动。代码块、表格和公式提供局部横向滚动；长题目和正文可断行。弹窗最大高度为 90dvh，普通 / 宽弹窗的最大宽度分别为 600px / 1020px，左右至少保留 16px。

矮窗口（宽 ≥681px、高 ≤850px）让展示标题使用 `clamp(64px, min(9.5vw, 16vh), 136px)` 和 1.06 行高，标题区自然高度，顶部间距 24px；真实主题保留 23px。文字流随标题区高度变化。普通高桌面仍保留原有大字构图。首页的本机保存说明和收束控制改用自然流布局；手机动作可换行，不把整张表单强行压进一屏。

**记录与验收边界：** 2026-09-13 已修复 critique 的全部五项优先问题并核对桌面、平板与手机模拟视口，验证记录见 `VALIDATION.md`。未执行物理手机、200% 文字缩放、真实 API 或长时间 TTS 验收。历史构建验收保留在 `VALIDATION.md` 与 `.impeccable/review/`，不作为本次通过结论。
...
主动作使用实心钴蓝与白字，最小高度 44px；次动作保持透明底、`#abb4c3` 边线；文字动作使用钴蓝。一般悬停为浅灰蓝，主动作悬停为更深蓝。禁用时透明度 0.45、不可用光标。按钮、链接和表单键盘焦点采用 2px 钴蓝轮廓，偏移 4px。状态色过渡为 180ms ease-out。开始研讨使用更宽、更高的主动作变体，并在尚未加载或问题为空时禁用。创建会话前检查实际分配给三位伙伴的模型配置及启用的语音密钥；缺少配置时按钮显示「配置后开始研讨」，打开设置且保留草稿。输入齐全只表示可以尝试连接，不等于已检测成功。动作旁提供「先看标注示例」。

### Navigation / Filters

桌面导航为文字，当前页由底部 2px 钴蓝线标识。手机的新增与记录入口使用图标；模型与声音入口保留可见名称。题库方向筛选是可横滚的文字标签，选中项加重并加底线；模式切换则是同一细边框内的实心蓝选中区。
...
普通输入为透明底、细边框、轻圆角；文字光标为钴蓝。研究主题是无框大输入，保留可见的「研究主题／可编辑」标签和快捷键提示，仍是可聚焦、可调整高度的 textarea。提示词使用内嵌浅底、多行编辑区；组合预览保留换行。已添加资料使用紧凑浅底标签与有名称的删除动作，长文件名可断行。标签内距、间距和删除按钮内距跟随首页布局单位，字号保持 13px。材料输入范围是 PDF、文字、链接。

### Agent Controls / Dialogs

主页三个伙伴控件显示名字、实际职责的两行摘要与编辑箭头；角色 B 的身份图标为青绿。角色编辑在宽弹窗中使用 192px 左侧选项栏与正文提示词区，支持名称、职责、共同规范、组合预览和恢复默认。配置即时保存的说明属于操作上下文。Settings 默认展示共享连接，将分别配置和角色分配按需展开；已有独立分配时自动展开。选项使用实际角色名称和模型名。设置、角色编辑和助教视图切换用 `aria-pressed` 表达选中状态，检测中和实际返回结果保持明确。弹窗基于原生 `<dialog>` 与 `showModal()`；关闭按钮、Escape 和弹窗矩形之外的遮罩点击均触发关闭。表单在手机变为单列，弹窗内容在最大高度内滚动。

### Reading Room

已有讨论首次从头阅读，后续在本机恢复上次滚动位置；新发言变化且用户跟随时才自动滚动。空助教欢迎区不滚动。「跳到最新发言」参与正常布局，避免与手机播放器重叠。连续讨论按轮次、角色名称、正文和行内工具组织；角色 A 为蓝、角色 B 为青绿。助教位于独立侧栏，移动端用覆盖面板。朗读高亮依附当前段落，桌面播放按钮在悬停或键盘焦点出现，手机始终可见。资料和笔记与助教共享安静的辅助面板语言。

### Feedback States

加载中的 `.spin` 图标采用 1s linear 循环旋转，减少动态效果时停止。设置检测结果使用浅蓝底与蓝色文字，错误框使用浅红底与深红文字；流式输出未完成与资料尚未验证使用暖色提示。手机保留全局「检查设置」，全屏助教另有就地错误与恢复入口；缺少配置时保留待发送问题。重点触控入口（筛选、轮数、角色编辑、视图切换等）至少 44px，手机表单输入至少 16px。以上反馈保留可读状态文案，具体局部配色以当前 CSS 为准，不将局部状态色扩张为品牌强调色。
